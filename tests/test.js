...
import complexCaps from '../examples/complex.json'

describe('Zoster instance tests', _ => {
  ...
  it('Test create_intent_url method', (done) => {
    let z = zoster({capabilities: complexCaps})
    if (z.createIntentUrl(complexCaps) !== 'intent://zoster/hello?user=vruno#Intentscheme=zosterpackage=com.urucas.zoster_testappend') {
  ...
  it('Test is_intenturl_ok method', (done) => {
    let z = zoster()
    let googIntentUrl = 'intent://zoster/hello?user=vruno#Intentscheme=zosterpackage=com.urucas.zoster_testappend'
    if (!z.isIntenturlOk(googIntentUrl)) {
      throw new Error('Fail is_intenturl_ok testing a good intent url')
    }
    // missing  between scheme value and package
    let badIntentUrl = 'intent://zoster/hello?user=vruno#Intentscheme=zosterpackage=com.urucas.zoster_testappend'
    if (z.isIntenturlOk(badIntentUrl)) {
  ...
  it('Test check_min_capabilities method', (done) => {
    let z = zoster()
    if (z.checkMinCapabilities({}) === true) {
      throw new Error('Fail check_min_capabilities, caps.pkg not setted and check_min_capabilities return true')
    }
    if (z.checkMinCapabilities({pkg: 'com.urucas.zoster_testapp'}) === true) {
      throw new Error('Fail check_min_capabilities, caps.scheme not setted and check_min_capabilities return true')
    }
    if (z.checkMinCapabilities({pkg: 'com.urucas.zoster_testapp', scheme: 'zoster'}) === true) {
      throw new Error('Fail check_min_capabilities, caps.action not setted and check_min_capabilities return true')
    }
    if (z.checkMinCapabilities({pkg: 'com.urucas.zoster_testapp',
      intentURL: 'intent://zoster/hello?user=vruno#Intentscheme=zosterpackage=com.urucas.zoster_testappend'}
      ) === true) {
  ...
  it('Test create_local_server_app creates app routes', (done) => {
    let z = zoster()
    let server = z.createLocalServerApp()
    ...
    if (!flag) {
      throw new Error('Method create_local_server_app fails to create static route')
    }
    ...
    if (!flag) {
      throw new Error('Method create_local_server_app fails to create main route')
    }
    ...
    if (!flag) {
      throw new Error('Method create_local_server_app fails to create upload route')
    }
    done()
  })
  ...
  it('Test create_local_server_test_site creates local test route', (done) => {
    let z = zoster()
    try {
      let server = z.createLocalServerTestSite()
      if (server === undefined) {
    ...
    let capsWithIntentUrl = caps
    caps.intentURL = 'intent://zoster/hello?user=vruno#Intentscheme=zosterpackage=com.urucas.zoster_testappend'
    // check test route is created when empty params
    let server = z.createLocalServerTestSite(capsWithIntentUrl)
    let express = server.__express__
    let routes = express._router.stack
    let flag = false
    for (let i = 0; i < routes.length; i++) {
    ...
    server = z.createLocalServerApp()
    let server1 = z.createLocalServerTestSite(capsWithIntentUrl, server.__express__, server)
